/* ===== OSKAR_FIREBASE_CACHE_RESET_BOOTSTRAP ===== */
(function(){
  'use strict';
  var PROJECT_ID = 'uiuii-2fb40';
  var ROOT_KEY = 'uiuii-2fb40';
  var VERSION = 'uiuii-2fb40-cache-reset-20260520-v2';
  var APP_KEY = 'supermarket_pos_ar_v1';
  var PROJECT_MARKER = 'oskar_active_firebase_project_id';
  var CACHE_MARKER = 'oskar_cache_version';
  function parse(v){try{return JSON.parse(v||'{}')||{};}catch(e){return {};}}
  function backupAndReset(reason){
    try{
      var oldDB = localStorage.getItem(APP_KEY);
      if(oldDB) localStorage.setItem('oskar_backup_before_'+PROJECT_ID+'_'+Date.now(), oldDB);
      localStorage.removeItem(APP_KEY);
      localStorage.removeItem('currentUser');
      localStorage.removeItem('oskar_current_branch_id');
      localStorage.removeItem('oskar_pending_sync_operations');
      localStorage.setItem('oskar_last_cache_reset_reason', reason || 'firebase-project-changed');
    }catch(e){}
  }
  try{
    var lastProject = localStorage.getItem(PROJECT_MARKER) || '';
    var db = parse(localStorage.getItem(APP_KEY));
    var companyKey = db && db.settings && db.settings.companyKey ? String(db.settings.companyKey) : '';
    if((lastProject && lastProject !== PROJECT_ID) || (companyKey && companyKey !== ROOT_KEY)) backupAndReset('old-project-or-company-key:' + (lastProject || companyKey));
    localStorage.setItem(PROJECT_MARKER, PROJECT_ID);
    localStorage.setItem(CACHE_MARKER, VERSION);
  }catch(e){}
  try{
    ...
    if('serviceWorker' in navigator){
      navigator.serviceWorker.getRegistrations().then(function(regs){
        regs.forEach(function(reg){
          try{ if(reg.active) reg.active.postMessage({type:'CLEAR_OSKAR_CACHE', version:VERSION}); }catch(e){}
          try{ reg.update(); }catch(e){}
        });
      }).catch(function(){});
    }
  }catch(e){}
  window.OSKAR_ACTIVE_FIREBASE_PROJECT = PROJECT_ID;
  window.OSKAR_ACTIVE_FIREBASE_ROOT = ROOT_KEY;
  window.OSKAR_CACHE_VERSION = VERSION;
})();
...
const firebaseConfig = {
  apiKey: "AIzaSyAUUX86cEzAkmDhuZSALOmUj7w1nOW20Fg",
  authDomain: "uiuii-2fb40.firebaseapp.com",
  databaseURL: "https://uiuii-2fb40-default-rtdb.firebaseio.com",
  projectId: "uiuii-2fb40",
  storageBucket: "uiuii-2fb40.firebasestorage.app",
  messagingSenderId: "649559908290",
  appId: "1:649559908290:web:1c540469839c346256c05b",
  measurementId: "G-MB0EZCHLXX"
};
const firebaseRoot = "uiuii-2fb40";
const fallbackRoots = ["uiuii-2fb40"];

(function(){
  'use strict';
  const APP_KEY='supermarket_pos_ar_v1';
  const ROOT_PATH=firebaseRoot;
  const DEFAULT_COMPANY=firebaseRoot;
  const LEGACY_COMPANY_KEYS=new Set(['SUPER-0001','OMAR-AD0BE-02','momen-5a28a','bdallah-7fa17','shop-d52dc','echo-store-cdgjhdvjt']);
  ...
  function rootList(){return [...new Set([ROOT_PATH].concat(Array.isArray(fallbackRoots)?fallbackRoots:[]).map(clean).filter(Boolean))]}
  function dataPath(companyKey,root=ROOT_PATH){
    const r=clean(root||ROOT_PATH), c=currentCompanyKey(companyKey);
    return r===c ? r : `${r}/${c}`;
  }
  function url(companyKey,root=ROOT_PATH){return `${firebaseConfig.databaseURL}/${dataPath(companyKey,root)}.json`}
  ...
  async function requestJSON(method,data,companyKey){
    ...
    let lastErr=null;
    const roots=method==='GET'?rootList():[ROOT_PATH];
    for(const root of roots){
      try{
        const r=await fetch(url(companyKey,root),options);
    ...
    throw lastErr || new Error('تعذر الاتصال بفايربيز');
  }
  ...
  async function patchPath(path,data,companyKey){
    ...
    const r=await fetch(`${firebaseConfig.databaseURL}/${dataPath(companyKey)}/${cleanPath}.json`,options);
...
/* ===== OSKAR_MOMEN_FINAL_FIREBASE_SYNC_PASSWORD_PATCH_V2 ===== */
(function(){
  'use strict';
  if(window.__OSKAR_MOMEN_FINAL_FIREBASE_SYNC_PASSWORD_PATCH_V2__) return;
  window.__OSKAR_MOMEN_FINAL_FIREBASE_SYNC_PASSWORD_PATCH_V2__ = true;

  var APP_KEY = 'supermarket_pos_ar_v1';
  var DEFAULT_ROOT = 'uiuii-2fb40';
  ...
  function rootKey(){ return DEFAULT_ROOT; }
  function readLocal(){ try{return JSON.parse(localStorage.getItem(APP_KEY)||'{}')||{};}catch(e){return {};} }
  function writeLocal(db){ db=normalizeDB(db||{}); localStorage.setItem(APP_KEY, JSON.stringify(db)); try{window.DB=clone(db);}catch(e){} return db; }
  function toast(msg){ try{ if(window.toast) window.toast(msg); else console.log(msg); }catch(e){ console.log(msg); } }
  function firebaseURL(path){
    path = String(path||'').replace(/^\/+|\/+$/g,'');
    return firebaseConfigFinal.databaseURL + '/' + rootKey() + (path?('/'+path):'') + '.json';
  }
  function normalizeError(err){
    var raw = cleanText(err && (err.firebaseBody || err.message || err));
    if(/permission_denied|Permission denied|PERMISSION_DENIED|401|403/.test(raw)) return 'صلاحيات Firebase ترفض القراءة أو الكتابة. افتح Realtime Database > Rules واجعل مسار '+DEFAULT_ROOT+' قابلًا للقراءة والكتابة أو فعّل تسجيل دخول Firebase.';
    if(/Failed to fetch|NetworkError|Load failed|تعذر الاتصال|network/i.test(raw)) return 'تعذر الاتصال بفايربيز. تأكد من الإنترنت، ومن أن الموقع يعمل عبر https أو localhost.';
    if(/404/.test(raw)) return 'رابط Realtime Database غير صحيح أو قاعدة البيانات غير مفعّلة.';
    return raw || 'تعذر تنفيذ العملية على Firebase';
  }
  async function request(method,path,data){
    var options = {method:method, cache:'no-store', headers:{'Content-Type':'application/json','Cache-Control':'no-cache','Pragma':'no-cache'}};
    if(data !== undefined) options.body = JSON.stringify(data==null?{}:data);
    try{
      var r = await fetch(firebaseURL(path), options);
      var body = '';
      try{ body = await r.text(); }catch(e){}
      if(!r.ok){ var er = new Error('Firebase '+r.status+' '+body.slice(0,220)); er.status=r.status; er.firebaseBody=body; throw er; }
      if(!body) return {};
  ...
  function normalizeDB(db){
    ...
    db.settings.companyKey = DEFAULT_ROOT;
    if(!db.settings.storeName) db.settings.storeName = 'كاشير أوسكار';
    if(!db.settings.currency) db.settings.currency = '₪';
    if(!db.settings.managerPassword) db.settings.managerPassword = DEFAULT_PASS;
    if(db.settings.managerPassword === DEFAULT_PASS) db.settings.forcePasswordChange = true;
    if(!Array.isArray(db.employees)) db.employees = [];
    return db;
  }
  function itemStamp(x){ return Date.parse((x&& (x._updatedAt||x.updatedAt||x._createdAt||x.createdAt||x.date)) || 0) || 0; }
  function mergeArrays(localArr, cloudArr, prefer){
    localArr = Array.isArray(localArr)?localArr:[]; cloudArr = Array.isArray(cloudArr)?cloudArr:[];
    var map = new Map(), noId=[];
    function add(x,src){
      if(!x || typeof x!=='object') return;
      if(x.id!=null && x.id!==''){
        var id=String(x.id), old=map.get(id);
        if(!old){ map.set(id, clone(x)); return; }
        var os=itemStamp(old), ns=itemStamp(x);
        if(ns>os || (ns===os && src===prefer)) map.set(id, Object.assign({}, old, clone(x)));
        else map.set(id, Object.assign({}, clone(x), old));
      }else{
        var key; try{key=JSON.stringify(x);}catch(e){key=String(noId.length);}
        if(!noId.some(function(y){try{return JSON.stringify(y)===key;}catch(e){return false;}})) noId.push(clone(x));
      }
    }
    cloudArr.forEach(function(x){add(x,'cloud');}); localArr.forEach(function(x){add(x,'local');});
    return Array.from(map.values()).concat(noId);
  }
  function settingsStamp(s){ return Date.parse((s&& (s._updatedAt||s.updatedAt)) || 0) || 0; }
  function mergeSettings(local, cloud, prefer){
    local=isObj(local)?local:{}; cloud=isObj(cloud)?cloud:{};
    var ls=settingsStamp(local), cs=settingsStamp(cloud), out;
    if(cs>ls) out = Object.assign({}, local, cloud);
    else if(ls>cs) out = Object.assign({}, cloud, local);
    else out = prefer==='cloud' ? Object.assign({}, local, cloud) : Object.assign({}, cloud, local);
    out.companyKey = DEFAULT_ROOT;
    if(!out.managerPassword) out.managerPassword = DEFAULT_PASS;
    if(out.managerPassword === DEFAULT_PASS) out.forcePasswordChange = true;
    return out;
  }
  function mergeDB(local, cloud, prefer){
    local=normalizeDB(clone(local)); cloud=normalizeDB(clone(cloud)); prefer=prefer||'local';
    var out = Object.assign({}, cloud, local);
    var keys = new Set(Object.keys(cloud).concat(Object.keys(local)));
    keys.forEach(function(k){
      if(Array.isArray(local[k]) || Array.isArray(cloud[k])) out[k] = mergeArrays(local[k], cloud[k], prefer);
    });
    out.settings = mergeSettings(local.settings, cloud.settings, prefer);
    out.lastSyncAt = now();
    out.lastLocalUpdate = out.lastLocalUpdate || local.lastLocalUpdate || now();
    return normalizeDB(out);
  }
  function markSettingsUpdate(db){
    db=normalizeDB(db); var t=now(); db.settings._updatedAt=t; db.lastLocalUpdate=t; return db;
  }
  function enforceRootInLocal(){
    var db = normalizeDB(readLocal());
    writeLocal(db);
    try{
      var u = JSON.parse(localStorage.getItem('currentUser')||'null');
      if(u && typeof u==='object'){ u.companyKey=DEFAULT_ROOT; u.managerId=u.managerId||'manager'; localStorage.setItem('currentUser', JSON.stringify(u)); }
    }catch(e){}
    return db;
  }
  ...
  async function putCloud(db){
    db = normalizeDB(db || {});
    return await request('PUT','',db);
  }
  async function patchCloud(path,data){
    return await request('PATCH',path,data||{});
  }
  async function verifiedPatchSettings(data){
    var t=now();
    data = Object.assign({}, data||{}, {companyKey:DEFAULT_ROOT, _updatedAt:t});
    await patchCloud('settings', data);
    var settings = await request('GET','settings');
    settings = isObj(settings)?settings:{};
    Object.keys(data).forEach(function(k){ if(String(settings[k]||'') !== String(data[k]||'')) throw new Error('لم يتم تأكيد حفظ '+k+' من Firebase'); });
    return settings;
  }
  async function updateManagerPassword(password){
    var pass = cleanText(password);
    if(!pass || pass.length < 6 || pass === DEFAULT_PASS) throw new Error('كلمة المرور الجديدة غير صالحة');
    var db = markSettingsUpdate(readLocal());
    db.settings.managerPassword = pass;
    db.settings.forcePasswordChange = false;
    db.settings.companyKey = DEFAULT_ROOT;
    writeLocal(db);
    if(!navigator.onLine) return db;
    try{
      await verifiedPatchSettings({managerPassword:pass, forcePasswordChange:false});
      var cloud = await getCloud();
      var merged = mergeDB(db, cloud, 'local');
      merged.settings.managerPassword = pass;
      merged.settings.forcePasswordChange = false;
      merged.settings.companyKey = DEFAULT_ROOT;
      await putCloud(merged);
      var verify = await request('GET','settings/managerPassword');
      if(String(verify) !== pass) throw new Error('لم يتم تأكيد كلمة المرور من Firebase');
      writeLocal(merged);
      lastError=null;
      return merged;
    }catch(e){ lastError=e; throw e; }
  ...
  var Bridge = {
    config: firebaseConfigFinal,
    root: function(){return DEFAULT_ROOT;},
    normalizeError: normalizeError,
    lastError: function(){return lastError;},
    async pullWithKey(){
      enforceRootInLocal();
      var local = readLocal(), cloud = await getCloud();
      if(!cloud || !Object.keys(cloud).length){ writeLocal(local); return local; }
      var merged = mergeDB(local, cloud, 'cloud');
      writeLocal(merged); lastError=null; return merged;
    },
    async pushWithKey(){
      var local = markSettingsUpdate(readLocal());
      var cloud = {};
      try{ cloud = await getCloud(); }catch(e){ cloud = {}; }
      var merged = mergeDB(local, cloud, 'local');
      await putCloud(merged);
      writeLocal(merged); lastError=null; return merged;
    },
    async sync(localDB, opts){
      if(!navigator.onLine) return writeLocal(localDB||readLocal());
      var local = normalizeDB(localDB || readLocal());
      if(opts && opts.prefer === 'cloud'){
        var cloudOnly = await getCloud();
        var pulled = mergeDB(local, cloudOnly, 'cloud');
        writeLocal(pulled); lastError=null; return pulled;
      }
      local.lastLocalUpdate = local.lastLocalUpdate || now();
      var cloud = {};
      try{ cloud = await getCloud(); }catch(e){ cloud = {}; }
      var merged = mergeDB(local, cloud, 'local');
      await putCloud(merged);
      var verify = await request('GET','lastSyncAt');
      if(!verify) throw new Error('لم يتم تأكيد المزامنة من Firebase');
      writeLocal(merged); lastError=null; return merged;
    },
    async pull(){return await this.pullWithKey(DEFAULT_ROOT);},
    async push(){return await this.pushWithKey(DEFAULT_ROOT);},
    async livePull(){return await this.sync(readLocal(),{prefer:'cloud'});},
    async updateManagerPassword(password){return await updateManagerPassword(password);},
    queueSync: function(db){
      clearTimeout(window.__OSKAR_MOMEN_FINAL_SYNC_TIMER__);
      window.__OSKAR_MOMEN_FINAL_SYNC_TIMER__ = setTimeout(function(){ if(navigator.onLine) Bridge.sync(db||readLocal()).catch(function(e){console.warn(e);}); }, 800);
    }
  };
  window.FirebaseBridge = Bridge;

  function installFinalSyncNow(){
    if(window.syncNow && window.syncNow.__momenFinalSyncPatch) return;
    var fn = async function(show){
      if(show === undefined) show = true;
      var btn = document.querySelector('.sync-button,.oskar-sync-btn,.top-actions button[onclick*="syncNow"],button[onclick*="syncNow"]');
      try{
        if(btn) btn.classList.add('syncing');
        if(!navigator.onLine){ if(show) toast('لا يوجد اتصال، البيانات محفوظة محليًا'); return; }
        var db = window.DB || readLocal();
        var merged = await Bridge.sync(db,{prefer:'local'});
        writeLocal(merged);
        try{ if(typeof window.saveDB === 'function') window.saveDB(merged); }catch(e){}
        if(show) toast('تمت المزامنة مع Firebase');
        try{ if(typeof window.updateSyncState === 'function') window.updateSyncState(); }catch(e){}
        try{ if(typeof window.renderPage === 'function') setTimeout(function(){window.renderPage();}, 60); }catch(e){}
        return merged;
      }catch(e){
        lastError=e; console.warn(e);
        if(show) toast('تعذر المزامنة: ' + normalizeError(e));
      }finally{
        if(btn) btn.classList.remove('syncing');
      }
    };
    fn.__momenFinalSyncPatch = true;
    window.syncNow = fn;
  }
  enforceRootInLocal();
  [0,80,250,700,1400,2600,4500].forEach(function(ms){ setTimeout(installFinalSyncNow, ms); });
  document.addEventListener('DOMContentLoaded', function(){ enforceRootInLocal(); installFinalSyncNow(); setTimeout(installFinalSyncNow,600); });
  window.addEventListener('load', function(){ enforceRootInLocal(); installFinalSyncNow(); setTimeout(installFinalSyncNow,900); });
  window.addEventListener('online', function(){ Bridge.queueSync(readLocal()); });
})();
